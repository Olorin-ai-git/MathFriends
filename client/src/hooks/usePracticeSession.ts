...
import { Difficulty, PracticeStats, MAX_ATTEMPTS_PER_PROBLEM } from '../../../shared/src/types/problems';
...
interface SessionState {
  ...
  isLoading: boolean;
  error: string | null;
  feedback: { correct: boolean; message: string; outOfAttempts?: boolean } | null;
...
export const usePracticeSession = create<SessionState>((set, get) => ({
      ...
      setDifficulty: (difficulty: Difficulty) => {
        set({ difficulty, currentProblem: null, currentAnswer: null, feedback: null, attemptsRemaining: MAX_ATTEMPTS_PER_PROBLEM, showingSelector: false });
      },

      showDifficultySelector: () => {
        set({ currentProblem: null, currentAnswer: null, feedback: null, attemptsRemaining: MAX_ATTEMPTS_PER_PROBLEM, showingSelector: true });
      },

      loadProblem: async () => {
        set({ isLoading: true, error: null, feedback: null, attemptsRemaining: MAX_ATTEMPTS_PER_PROBLEM });
        try {
          ...
          const answer = 'answerX' in problemData
      ...
      submitAnswer: async (userAnswer: number) => {
        const { currentProblem, currentAnswer, stats, difficulty, attemptsRemaining } = get();
